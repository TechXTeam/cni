...
[Plugin specific fields](https://github.com/TechXTeam/cni/blob/master/SPEC.md#network-configuration) formed part of the original CNI spec and have been present since the initial release.
...
For example, the configuration for a port mapping plugin might look like this to an operator (it should be included as part of a [network configuration list](https://github.com/TechXTeam/cni/blob/master/SPEC.md#network-configuration-lists).
```json
{
  "name" : "ExamplePlugin",
  "type" : "port-mapper",
...
`args` in [network config](https://github.com/TechXTeam/cni/blob/master/SPEC.md#network-configuration) were reserved as a  field in the `0.2.0` release of the CNI spec.
